...
const RecipeScoreIcon = ({ recipeId }) => {
    ...
    const [averageScore, setAverageScore] = useState(() => {
        // Attempt to load the average score from local storage first
        const savedScore = localStorage.getItem(`score_${recipeId}`);
        return savedScore || 'Loading...';
    });
        ...
        fetch(`${API_BASE_URL}/api/score/average/${recipeId}`)
            ...
            .then(data => {
                if (data.averageScore !== undefined) {
                    ...
                    localStorage.setItem(`score_${recipeId}`, data.averageScore);
                } else {
                    setAverageScore('n/a');
                }
            })
